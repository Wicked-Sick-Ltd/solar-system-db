...
| Crawler | Starts on llm1 at 1 rps against the published file at `~/deploy/solar-data/solar_system.sqlite` (the builder installs it there after a verified publish). Craig emails JPL (`ssd-api` contact) offering the tool and asking for a rate for `192.33.132.253`; `CRAWLER_RPS` rises only after a reply. | Unchanged policy; the host changed. |
